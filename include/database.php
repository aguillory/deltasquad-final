	...
	class MySQLDB
	{
		...
		*/
        function addEvent2($title, $type, $course, $crn, $seats, $notes, $dateStart, $dateEnd, $room, $user, $series, $time, $conflict){
			...
			if ($conflict < 1){
				...
				$message = "An event was created that needs your attention. Either the event conflicts with another, the room does not have sufficient capacity, or the event was created outside of the scheduled deadline. The event is ".$title.". <a href=\'./index.php?o=4&m=".$month."&a=".$day."&y=".$year."&cm=1\'>Please click here for more information and to approve or reject the event.</a>";
				$this->mailIt($message);
			}
			foreach ($crns as $c){
		...
		function addEvent2A($title, $type, $course, $crn, $seats, $notes, $dateStart, $dateEnd, $room, $user, $series, $time, $repeat, $repeatm, $repeatt, $repeatw, $repeatth, $repeatf, $re, $conflict){
			...
			foreach ($crns as $c){
				...
				$result = mysql_query($q, $this->connection);				
				if ($approval == "pending"){
					array_push($conEventArr, mysql_insert_id($this->connection));
				}
				...
				if ($repeatm == 1){
					...
					while ($re1 > $dateStartA) {
						...
						//repeat the entire conflict process over again.
						$q = "SELECT count(*) as Conflicts FROM event AS e WHERE series != 9100 AND room_number = '$room' AND room_number != 'Offsite' AND (
						(e.dateStart <= '$dateStart1' AND e.dateEnd >= '$dateStart1')
						OR
						(e.dateStart >= '$dateStart1' AND e.dateEnd <= '$dateEnd1')
						OR
						(e.dateStart >= '$dateStart1' AND e.dateStart <= '$dateEnd1')
						OR
						(e.dateStart <= '$dateStart1' AND e.dateEnd >= '$dateEnd1'))";
						//$myfile = fopen("error.txt", "a") or die(print_r($q));
						$result = $this->query($q);
						...
						if ($num_rows > 0){
							for($i=0; $i<$num_rows; $i++){
								$conflict  = mysql_result($result,$i,"Conflicts");
							}
						}
						$deadline = "select CASE WHEN COUNT(*) > 0 THEN 1 ELSE 0 END AS tail FROM ".TBL_DEADLINES." WHERE (CURRENT_TIME() BETWEEN open AND close) AND type='schedule'";
						$deadline2 = $this->query($deadline);
						$tail = mysql_result($deadline2,0,"tail");
						if ($tail == 0){
							$conflict++;
						}
						$cap = "SELECT capacity FROM ".TBL_ROOMS." WHERE room_number = ".$room;
						$cap2 = $this->query($cap);
						$cap3 = mysql_result($cap2,0,'capacity');
						if ($seats > $cap3){
							$conflict++;
						}
						if ($conflict < 1){
				...
				array_shift($conEventArr);
				if (sizeof($conEventArr)<1){
				$logMsg = "Event series created with no conflicts: ".$title;
					} else {
					$logMsg = "Event series created with some conflicts: ".$title;
					$URL = "";
					$URL2 = "";
					foreach ($conEventArr as $a){
						$eventid = $a;
						$URL = $URL."<a href=\'/viewconflict.php?e=".$eventid."\'>Event # ".$eventid."</a>.<br>";
						//$myfile = fopen("error.txt", "a") or die(print_r($URL));
					}
					foreach ($conEventArr as $a){
						$eventid = $a;
						$URL2 = $URL2."<a href=\'/showevent.php?e=".$eventid."&s=".$series."\'>Event # ".$eventid."</a>.<br>";
					}
					$findUser = "SELECT username FROM ".TBL_USERS." WHERE CWID = ".$user;
					$findUser = mysql_query($findUser, $this->connection);
					$findUser = mysql_result($findUser,0,"username");
					$mailInstructorSub = "One or more of your events have caused a conflict.";
					$mailInstructorBod = "One or more of the events you created have caused a conflict. Click the following event ID\'s to see what they are. <br>".$URL2;
					$message = "Event(s) were created that need your attention. Either the event conflicts with another, the room does not have sufficient capacity, or the event was created outside of the scheduled deadline. The event is titled ".$title.". The following are links to each conflict. <br>".$URL;
					$this->logIt($logMsg);
					$this->mailIt($message);
					$this->sendReply($findUser, $mailInstructorSub, $mailInstructorBod, ''.SCHED_ADMIN.'');
		...
		*/
		...
		function reject($eventid){
			...
			$this->sendReply($to,'Your event has been rejected.',$body,''.SCHED_ADMIN.'');
			$q = sprintf("DELETE FROM ".TBL_EVENTS." WHERE event_id = $eventid");
			$result = mysql_query($q, $this->connection);
		...
		function sendReply($mailTo,$mailSubject,$mailBody,$mailFrom){
			...
			date_default_timezone_set('America/Chicago');
			$date = date('m/d/Y')." at ".date('g:i.s')." ".date('a');
			$q = sprintf("INSERT INTO mail (UserTo, UserFrom, Subject, Message, SentDate, status) VALUES ('$mailTo','$mailFrom','$mailSubject','$mailBody','$date','unread')");
			$result = mysql_query($q, $this->connection);
		}
