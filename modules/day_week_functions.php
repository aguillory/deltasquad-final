	...
	function showGrid($date) {
		...
		if(!$session->isInstructor() & !$session->isAdmin()){
			if (isset($_GET['cwid'])&&$_GET['cwid']!=0&&$_GET['cwid']!=NULL){
				...
				$q = sprintf("SELECT DISTINCT ".TBL_EVENTS.".*, ".TBL_CRN.".course_number AS cnum from ".TBL_EVENTS." join ".TBL_CRN." ON ".TBL_EVENTS.".crn = ".TBL_CRN.".crn join ".TBL_SCHED." on ".TBL_SCHED.".crn = ".TBL_EVENTS.".crn where (".TBL_SCHED.".cwid = $studentCWID OR series=9100 OR ".TBL_EVENTS.".crn=0) AND CAST(dateStart AS DATE) = CAST('$dateNew' AS DATE) AND status='approved'");
				} else {
			...
			if (isset($sem) && ($sem != 0) && ($sem != NULL)){
				$q = sprintf("select  ".TBL_EVENTS.".*, ".TBL_COURSE.".course_number AS cnum from ".TBL_EVENTS." join ".TBL_CRN." ON ".TBL_EVENTS.".crn = ".TBL_CRN.".crn join ".TBL_COURSE." on ".TBL_COURSE.".course_number = ".TBL_CRN.".course_number where CAST(dateStart AS DATE) = CAST('$dateNew' AS DATE) AND (semester = $sem OR semester=0 OR series=9100 OR ".TBL_EVENTS.".crn=0)");
				} else if (isset($rm) && ($rm != 0 || $rm != '0') && ($rm != NULL)){
				...
				$q = sprintf("select DISTINCT ".TBL_EVENTS.".*, ".TBL_COURSE.".course_number AS cnum from ".TBL_EVENTS." join ".TBL_CRN." ON ".TBL_EVENTS.".crn = ".TBL_CRN.".crn join ".TBL_COURSE." on ".TBL_COURSE.".course_number = ".TBL_CRN.".course_number where (".TBL_CRN.".instructor = $CWID OR ".TBL_COURSE.".Lead_Instructor = $CWID OR series=9100 OR ".TBL_EVENTS.".crn=0) AND CAST(dateStart AS DATE) = CAST('$dateNew' AS DATE)");
			}
			//if admin
			} else {
			$sem = $_GET['sem'];
						...
						for ($j = 0; $j <= $i; $j++){
							...
							if ($removed[8] != 9100 && $removed[0] != ""){
								...
								if ($session->isAdmin() && $cm == 1){
									echo "<a href='./viewconflict.php?e=".$removed[7]."'>Conflict Manager</a>";
								}
								echo "<div class=\"title\"><a href='/showevent.php?e=".$removed[7]."&s=".$removed[8]."' ".$removed[10].">".$removed[11];
								echo " ".$removed[2]."</a><br>".$removed[3]."</div>\n";
